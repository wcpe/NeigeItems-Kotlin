...
object Give {
    ...
    // ni give [玩家ID] [物品ID] (数量) (绑定玩家) (是否反复随机) (指向数据) > 根据ID给予NI物品
    val give = subCommand {
        ...
        dynamic {
            ...
            dynamic {
                ...
                execute<CommandSender> { sender, context, argument ->
                    giveCommandAsync(
                        sender = sender,
                        player = Bukkit.getPlayerExact(context.argument(-1)),
                        id = argument,
                        amount = "1"
                    )
                }
                ...
                dynamic(optional = true) {
                    ...
                    execute<CommandSender> { sender, context, argument ->
                        giveCommandAsync(
                            sender = sender,
                            player = Bukkit.getPlayerExact(context.argument(-2)),
                            id = context.argument(-1),
                            amount = argument
                        )
                    }
                    ...
                    dynamic(optional = true) {
                        ...
                        dynamic(optional = true) {
                            ...
                            execute<CommandSender> { sender, context, argument ->
                                giveCommandAsync(
                                    sender = sender,
                                    player = Bukkit.getPlayerExact(context.argument(-4)),
                                    id = context.argument(-3),
                                    amount = context.argument(-2),
                            ...
                            dynamic(optional = true) {
                                ...
                                execute<CommandSender> { sender, context, argument ->
                                    giveCommandAsync(
                                        sender = sender,
                                        player = Bukkit.getPlayerExact(context.argument(-5)),
                                        id = context.argument(-4),
                                        amount = context.argument(-3),
                                        bindPlayer = context.argument(-2),
                                        random = context.argument(-1),
                                        data = argument
                                    )
                                }
                            }
    ...
    // ni giveSilent [玩家ID] [物品ID] (数量) (绑定玩家) (是否反复随机) (指向数据) > 根据ID给予NI物品
    val giveSilent = subCommand {
        ...
        dynamic {
            ...
            dynamic {
                ...
                // ni giveSilent [玩家ID] [物品ID] (数量) (绑定玩家)
                dynamic(optional = true) {
                    suggestion<CommandSender>(uncheck = true) { _, _ ->
                        arrayListOf("amount")
                    }
                    ...
                    // ni giveSilent [玩家ID] [物品ID] (数量) (绑定玩家) (是否反复随机)
                    dynamic(optional = true) {
                        suggestion<CommandSender>(uncheck = true) { _, _ ->
                            arrayListOf("true", "false")
                        }
                        ...
                        // ni give [玩家ID] [物品ID] (数量) (绑定玩家) (是否反复随机) (指向数据)
                        dynamic(optional = true) {
                            suggestion<CommandSender>(uncheck = true) { _, _ ->
                                arrayListOf("data")
                            }
            ...
            dynamic(optional = true) {
                ...
                dynamic(optional = true) {
                    ...
                    dynamic(optional = true) {
                        ...
                        execute<CommandSender> { sender, context, argument ->
                            giveAllCommandAsync(
                                sender, context.argument(-3), context.argument(-2), context.argument(-1), argument
                            )
                        }
                    }
                }
    ...
    private fun giveCommandAsync(
        ...
        async {
            giveCommand(sender, player, id, amount, random, data, tip, bindPlayer)
        }
    }
                            ...
                            if (tip) {
                                ...
                                player.sendLang(
                                    "Messages.givenInfo", mapOf(
                                        Pair("{amount}", amount.toString()), Pair("{name}", itemStack.getParsedName())
                                    )
                                )
                            }
                            // 未知物品ID
